...
The latter is concise and neutral by design, devoid of any trace of the authors' personality.

While rigor is key in a paper with theorems and proofs, it has the unfortunate side effect of being unrelatable. A common reaction is _"How did they come up with that? I could never."_
...
I'll choose a paper that was particularly _fun_, [Taming the Knight's Tour: Minimizing Turns and Crossings](https://arxiv.org/pdf/1904.02824.pdf), and which was cited by [Donald Knuth](https://en.wikipedia.org/wiki/Donald_Knuth) (joint work with Juan Jose Besa, Timothy Johnson, Martha C. Osegueda, and Parker Williams).
...
<Callout>
  Funnily enough, there is one person who'd later become interested in the problem of finding the 8x8 tour with the fewest turns: Donald Knuth. And unlike us, he succeeded. He presented the tour in his 29th Christmas Lecture (2025), [Adventures with Knight's Tours](https://www.youtube.com/watch?v=MKiRte-tnMY), which I attended and where I snapped a picture of it:

<BlogImage
  src="/blog/knights-tour/knuths_tour.png"
  alt="Donald Knuth's 8x8 tour"
  width="60%"
/>

Our paper eventually got cited by Knuth (read [here](https://x.com/Nil053/status/2067322147819946361) how it happened and my thoughts on the lecture).
...
Now, the _"only"_ thing that was left to get a tour with `O(n)` turns was completing the edges of the board. While obviously hard, it didn't seem outright impossible.
...
## More unexpected help

Five years later, it happened again. Shisheng Li, from USTC, worked through Knuth's Exercise 7.2.2.4-161, which cites our paper, and built upon it.

He made another heel optimization: every five adjacent copies can be replaced by a single 4x40 template that keeps the same knight formation but has 10 fewer crossings (see his [interactive visualization](https://daizisheng.github.io/MinCrossingsKnightsTour/index.html)).

<BlogImage
  src="/blog/knights-tour/shisheng-substitution.png"
  alt="Top: five adjacent 4x8 heels from our construction, each outlined in green. Bottom: the same 4x40 region covered by a single SequenceP_40 template outlined in red, with fewer crossings"
  width="100%"
/>

The number of crossings goes down from `12n + O(1)` to `11.5n + O(1)`.
