...
We start by finding _any_ two node-disjoint paths from `s` to `t`, as described above. Which we'll call the _red path_ (`RP`) and the _blue path_ (`BP`). We'll call the edges in `RP` _red edges_ and the edges in `BP` _blue edges_.
...
Thus, we can narrow down the set of candidate edges to those in `RP` and `BP`.
...
Similarly, we can find all the blue candidates, which we'll call `BC = (b1, b2, ..., bk')`, by removing all the red edges from `G` and finding the bridges in the remaining graph that overlap with `BP`:
...
We call `RCC_i` the connected component in `G - RC - BP` that contains the red segment `rs_i`. Thus, the red path traverses `RCC_0, RCC_1, ..., RCC_k`, in this order. We also do the same for the blue path, and call the components `BCC_0, BCC_1, ..., BCC_k'`.
