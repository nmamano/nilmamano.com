...
In a [previous post](/blog/wall-game-ai), I described the AlphaZero-style AI behind the Wall Game ([wallgame.io](https://wallgame.io)) at the architectural level: Monte Carlo Tree Search (MCTS), the neural network, training via self-play, inference, deployment, etc.
...
**[Folly](https://github.com/facebook/folly)** (Facebook Open-source Library) is Meta's C++ foundation library. C++'s coroutine support is barebones, so we rely on Folly for the runtime. Here's what we use from it:
    ...
    - **`MPMCQueue`** and **`EvictingCacheMap`**: A lock-free queue and an LRU cache, respectively. Both explained at point of use below.
...
`BatchedModel::inference()` creates an `InferenceTask`, which bundles the input state with a promise, enqueues the task, and returns the `SemiFuture` for that promise. It is _not_ a coroutine - it's a regular function that hand back the `SemiFuture` so the *caller* can wait.
...
A single MCTS sample produces one inference request. To fill GPU batches, we need many concurrent requests. Here's how they're generated:
...
This creates `samples` coroutines (e.g., 1,200) but `collectAllWindowed` only allows `max_parallelism` to be active at a time. Each active coroutine traverses the tree, reaches a leaf, and suspends on `co_await` for GPU evaluation - at which point `collectAllWindowed` starts the next one. At any given moment, many coroutines are suspended waiting for GPU results, which is what fills the inference batch.

The concurrency cap exists because MCTS samples aren't independent - each one refines the tree, so later samples benefit from earlier ones. To prevent parallel samples from going down the same branch, each sample temporarily increments an `active_samples` counter on the edges it traverses. The UCB formula penalizes edges with active samples, steering concurrent samples toward less-explored branches (a technique known as _virtual loss_). Despite this, too many parallel samples still interfere with each other, leading to redundant inferences.

During self-play, the GPU batch is primarily filled by requests from hundreds of parallel games, so per-game parallelism can be kept low. In production, where fewer games run concurrently, more parallel samples per game are needed to keep the GPU utilized.

## Final Thoughts

It's 2026 and _model inference_ is, quite literally, a significant chunk of the economy. When we think of models like GPT-5, we often focus on the months-long training run; but it is at _inference time_ when AI companies actually make money. That's why it's so important and only becoming more relevant.

Understanding the inference pipeline for our ResNet-style model is a great starting point. It introduces many important concepts and optimizations: pinned memory, pipelining, etc. - see the feature list at the top of the post.

LLM inference, however, is another beast:
...
In contrast, I found that trying to run local LLMs on that very same hardware is useless. The models that fit in it are bad, and inference is slow.

The experiment only left me with a deep appreciation for the speed and volume at which the frontier AI labs are capable of running inference with much larger models. I think some really interesting engineering work must be happening there.
