---
title: "Training an AlphaZero-Style AI for a New Board Game"
date: "2026-02-16"
excerpt: "A playbook for building AlphaZero-style AIs, from architecture to training to deployment."
coverImage: "/blog/wall-game-ai/cover.png"
categories: ["wallgame", "ai", "swe"]
---

<Callout>
  Here is part 2: [Deep Dive: Inference Pipeline for
  Self-Play](/blog/wall-game-inference).
</Callout>

I created [wallgame.io](https://wallgame.io), an online board game. Since it's a two-player game, I wanted a human-level AI so anyone could play at any time.

I built a traditional, Minimax-based engine, while my friend [Thorben](https://github.com/t-troebst) built an AlphaZero-style AI: a neural network trained through self-play, guided by Monte Carlo Tree Search, running on a consumer GPU.
...
This post describes the design and can serve as a reference. We cover the architecture, implementation, training, and server integration.

Note: the network architecture described here is the ResNet from the AlphaZero paper, which is what the engine used originally. The ResNet was later superseded by a transformer, which is covered in [this section](#update-july-2026-replacing-the-resnet-with-a-transformer).

<Toc />

## Links

- The game and the AlphaZero-based AI are live at [wallgame.io](https://wallgame.io). If you manage to beat the **Hard Bot**, let me know! 
- Thorben's original engine: [Deep-Wallwars](https://github.com/t-troebst/Deep-Wallwars). Credits to him for re-implementing the AlphaZero paper from scratch!
- The game has a [monorepo](https://github.com/nmamano/wallgame) with:
    ...
    - [Updated version of Thorben's engine](https://github.com/nmamano/wallgame/tree/main/deep-wallwars)
    - A [dummy engine](https://github.com/nmamano/wallgame/tree/main/dummy-engine) for testing
- The game has a dev blog, [nilmamano.com/blog/category/wallgame](/blog/category/wallgame) including this post.
- The [minimax-based C++ engine](https://github.com/nmamano/wallwars/tree/master/AI) can be played against in the old site: [wallwars.net](https://wallwars.net).
- References: the [AlphaZero paper](https://arxiv.org/abs/1712.01815) by Silver et al. (2017) and the [ResNet paper](https://arxiv.org/abs/1512.03385) by He et al. (2015). 

## The Wall Game

The Wall Game is a turn-based game played on a rectangular grid. Each player controls a cat and a mouse, which start at the corners, and the goal is to catch the opponent's mouse before they catch yours.

<BlogImage
  src="/blog/wall-game-ai/start-and-final-pos.png"
  alt="Side by side: the starting position with pieces in their initial spots, and a final position after a full game with walls placed across the board."
/>

On each turn, you take 2 actions. Each action can be moving your cat, moving your mouse, or placing a wall. Cats and mice move to adjacent cells, but walls can be placed between any two cells on the board. The only restriction is that you **cannot completely block** the opponent's cat from reaching your mouse. The full rules are at [wallgame.io/learn](https://wallgame.io/learn).

<BlogImage
  src="/blog/wall-game-ai/actions.png"
  alt="The four action types: cat move, mouse move, vertical wall placement, and horizontal wall placement."
  width="80%"
/>

Watch this [replay of a full game](https://wallgame.io/game/f2v5G_nC) to get a feel for the tactics that emerge. You can see more games in action in the landing page's game showcase ([wallgame.io](https://wallgame.io)).

### Why it is Hard for AI
...
For the [Minimax engine](https://github.com/nmamano/wallwars/tree/master/AI), I implemented all the classic techniques: alpha-beta pruning, transposition tables, and even some clever [graph theory algorithms](https://nilmamano.com/blog/double-edge-cut-problem) for efficient move generation, but it doesn't scale.

What crushed it is the branching factor. On an `RxC` board, there are about `2*R*C` possible walls. With *two* actions per turn, the number of possible moves is roughly the square of that.

In contrast, the AlphaZero-based engine is superhuman on `12x10` boards, handling raw branching factors of approximately `(2*12*10 + 8)^2 = 61504`. Chess has average branching factor `~40`.
...
AlphaZero follows an elegant solution known as Upper Confidence Bound (UCB). To use UCB, for each option, you need two things: the expected payoff and the expected variance (in MCTS, we estimate those based on visit counts and average evaluation). The option with the highest payoff maximizes exploitation; the one with the most variance maximizes exploration.
...
<BlogImage
  src="/blog/wall-game-ai/self-play-loop.svg"
  alt="The self-play training loop: Self-Play produces Game Data, which trains the Network, producing an Updated NN that feeds back into Self-Play."
  width="60%"
/>

## Neural Network Architecture

Here we describe our first model, which worked for boards up to 8x8.

Our network follows the AlphaZero architecture: a ResNet core with 20 residual blocks and 128 hidden channels, totaling about 6.3 million parameters. Small by modern deep learning standards, but large enough to capture the strategic complexity of an 8x8 board game.
...
Update (July 2026): the engine has since moved from ResNet to a transformer, which trained faster and generalizes across board sizes and variants. See [Replacing the ResNet with a Transformer](#update-july-2026-replacing-the-resnet-with-a-transformer) at the end of the Training section. The ResNet described here is what the original 8x8 models used.
...
|-------|-------------------|--------|
| 0 | Distance from your cat to every cell | 0 at cat's position; less than 1 for reachable cells, increasing with distance; 1 for unreachable cells |
...
After 750,000 self-play games, the result is a superhuman player that moves almost instantly, even though the Elo didn't stagnate yet. Its style is recognizably strategic but unpredictable to me and other "strong" human players.

<BlogImage
  src="/blog/wall-game-ai/classic_model_elo.png"
  alt="Elo progression over generations for the 8x8 Classic model."
  width="80%"
/>
...
<BlogImage
  src="/blog/wall-game-ai/mouse_move_evolution.png"
  alt="Standard variant training: Elo progression and mouse move fraction over generations."
  width="80%"
/>

The warm-start paid off: the model became strong in far fewer generations.

### The Universal Model, First Attempt: Warm-Starting

Next, I wanted models that could play the standard and classic variants for larger boards (up to 12x10).

My first attempt was to keep warm-starting, the trick that had just worked so well for Standard. It didn't get there, but it's worth walking through, because where it falls short is what motivated the approach that did work.
...
So the goal became a universal model: a single network that plays both Classic and Standard on larger boards (12x10), so all compute goes toward improving one model.
...
- **Half-and-half self-play:** Each generation plays half its games as Classic and half as Standard. The variant indicator (plane 8) tells the network which variant it's playing, ensuring the model learns to use that signal from the start. The strength for the two variants evolved at a similar pace:

<BlogImage
  src="/blog/wall-game-ai/universal_model_elo.png"
  alt="Elo progression for the universal model across variants."
  width="80%"
/>

This universal model didn't get to human strength.

The approach that eventually did work was a different one: a network with no board-size-dependent weights, trained from scratch on a mix of board sizes. That's the transformer, below.
...
Everything above uses the ResNet architecture from the AlphaZero paper. Recently, I retrained the engine from scratch with a transformer instead.

The idea came from the [ViT (Vision Transformers) paper](https://arxiv.org/abs/2010.11929), which shows that transformers can work better for vision than CNNs. This made me wonder if the same would apply to the Wall Game. I checked if anyone has used transformers with the AlphaZero-style tree search, and, indeed, Leela Chess Zero's transformer policy jumped ~270 Elo over the CNN.

Where the ResNet sees the board as a grid of pixels, the transformer sees it as a sequence of vectors. Each cell becomes one vector of 256 numbers, with learned row and column embeddings added so the model knows which cell it is. On a 12x10 board that's 120 vectors, and we prepend one extra vector that belongs to no cell, for 121 in total.

Those 121 vectors go through 10 encoder layers - each one an attention block followed by an MLP block - and 121 vectors come out. The model is about 8 million parameters, compared to the ResNet's 6.3 million.

Then we choose which output vector to read each prediction from. A wall placement belongs to a cell, so each cell's output vector gives the two wall logits for that cell. But "who is winning" and "which direction should the cat move" belong to the position as a whole, and no cell is a natural home for them - that's what the extra vector is for. Having attended to every cell at every layer, it ends up holding a summary of the whole board.[^clstoken]

[^clstoken]: It's the same device as the `[CLS]` token in BERT, and the same thing GPT does when it reads its next-word prediction off the last token's output vector: one slot that attention has loaded with global information.

<BlogImage
  src="/blog/wall-game-ai/transformer-tokens.svg"
  alt="121 vectors go into 10 encoder layers and 121 come out; the cell vectors give wall logits, the extra vector gives the evaluation and pawn moves."
  width="85%"
/>

The rest of the pipeline - MCTS, the self-play loop, the loss - is unchanged.

This time, I tried a different approach to turn the transformer into a universal model: train the model from scratch, and progressively expand the mix of board dimensions that are trained at each generation. I started with 50% Classic and 50% Standard on 8x8 boards. When it surpassed the ResNet models, I started adding 8x9, 9x8, and 9x9 boards into the mix, while still keeping 25% of games at 8x8 Classic. This way, it not only preserved the strength at 8x8, but it kept getting better.

Eventually, I expanded to board sizes up to 12x10, and the share of 8x8 Classic games dropped below 10%, yet the model kept improving. That single model is what runs on wallgame.io today, with superhuman strength across every variant and board size.

<BlogImage
  src="/blog/wall-game-ai/cnn_vs_transformer.png"
  alt="Top: Elo progression for the ResNet and the transformer against total self-play games. Bottom: the percentage of self-play games played on 8x8 Classic for each."
  width="90%"
/>

I later plotted the same run mode by mode: [Elo across all 126 generations for 10 game modes](/posts/wallgame-transformer-elo-across-126-generations). The clearest moment there is generation 96, where the animal cycle variant enters the training mix, a variant that did not exist when I started training.

One caveat, in fairness to the ResNet: it had a training bug whose impact is impossible to quantify. For half of the positions in every generation (those played by Player 1), instead of training on the rich move distribution found by MCTS, it trained on only the selected move, i.e., a distribution with probability 1 at that move and 0 everywhere else.
...
<BlogImage
  src="/blog/wall-game-ai/integration-architecture.svg"
  alt="Integration architecture: Browser connects via WebSocket to Game Server (Fly.io), which connects via WebSocket to Bot Client (GPU machine), which communicates via stdin/stdout JSON-lines with the C++ Engine."
  width="100%"
/>
...
The bot client is configured via a JSON file that specifies the bot's name, appearance (avatar, colors), engine parameters, and a command to launch the binary for each engine. This is what makes it engine-agnostic. Here is the [official bot's config](https://github.com/nmamano/wallgame/blob/main/official-custom-bot-client/deep-wallwars.prod.config.json).
...
I'm working on a background process that analyzes past games played on the site and identifies positions that make for **interesting puzzles**. Instinctively, a puzzle-like position is one where there is a short tactical sequence that yields a decisive advantage. A good heuristic for whether a move is non-obvious is that the human player in the actual game didn't find it. To operationalize this, we can lean on the superhuman engine.
...
_Want to leave a comment? You can post under the [linkedin post](https://www.linkedin.com/posts/nilmamano_training-an-alphazero-style-ai-for-a-new-activity-7430007053164822528-cxoO) or the [X post](https://x.com/Nil053/status/2024242486361849872)._
