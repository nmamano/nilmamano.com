...
Where it gets interesting is if we are allow modifying the input grid to save extra space.
...
On the one hand, DFS is recursive, and the call stack still counts as extra space. In the worst case, we could zig-zag through the entire grid with DFS, making `O(R * C)` nested calls.[^2]
...
We can see how an 'L' shaped wall allows us to split one corner of the grid into four smaller corners.
