...
Surprisingly, it turns out that the BFS queue may grow to `Θ(min(R, C)^2)` in the worst case.

To show this, we need to construct a very specific grid arrangement that maximizes the number of cells equidistant from the starting cell.
The construction follows a recursive pattern.[^3]

[^3]: Credit to [Timothy Johnson](https://www.linkedin.com/in/timothy-johnson-33659aa2/) for this idea.
...
<BlogImage
  src="/blog/counting-islands/fractal127.png"
  alt="Fractal"
  width="100%"
/>
...
<BlogImage
  src="/blog/counting-islands/construction.png"
  alt="Recursive construction"
  width="60%"
/>

We can see how one corner of the grid gets split into four smaller corners.
...
1. If the grid is thin or tall (many more rows than columns or vice versa), `O(min(R, C)^2)` is better than `O(R*C)`.
2. The worst-case for BFS is a lot more contrived, so BFS will use less space on _average_ or on _expectation_.
