...
With my first (careless) implementation, there wasn't a clear source of truth for the timers, so if a player moved right before their timer run out, they could thing the game kept going while the opponent saw that they timed out.
