...
<Callout>
  ...
  I'm making. However, something probably more interesting came out of it: an
  assessment of frontend generators' ability to one-shot a 6000-word design doc
  prompt. The results deeply impressed me. They are in an appendix at the
  bottom. Feel free to skip the design doc and check out the
  [appendix](#appendix).
</Callout>
...
[bolt.new](https://bolt.new/) had great prompt adherence, about the same as v0.dev, but the site looked generic like lovable's (and the board was also messed up). Other tools had a prompt length limit, so I didn't even try.
