...
This post covers how I built [isomux.com](https://isomux.com) ([github](https://github.com/nmamano/isomux)) and the lessons I learned about designing meta-harnesses.

I describe independently-developed features like [inter-agent skills](#applications), "Google Docs"-style [live collaboration](#human-collaboration), [meta-harness reflection](#meta-harness-reflection), [agent-driven UI](#giving-agents-ui-superpowers), [hierarchical agents](#hierarchical-agents), [an agent-human shared task board](#agent-human-shared-task-board), and (in a standalone post) [a personal-software suite](/blog/personal-software-suites).

For a closer look at the UX side, see [Making Agents User-Friendly](/blog/agent-ui).

<Toc />

## Introduction

I went through every level of [Steve Yegge's hierarchy](https://steve-yegge.medium.com/welcome-to-gas-town-4f25ee16dd04).
...
<BlogImage
  ...
  alt="My weekly commit count from Nov 2024 to Sep 2026, banded by stage of Yegge's hierarchy: Level 4 with Cursor as daily driver, Level 5 with Claude Code from Oct 2025, and Levels 6-8 with Isomux from Mar 2026. Weekly commits climb from single digits to a peak near 250"
/>
...
<DemoEmbed
  src="https://isomux.com/demo?embed"
  href="https://isomux.com/demo"
  title="Isomux live demo"
/>
...
The idea is that **by anthropomorphizing agents, we make them accessible to non-technical users**; coordinating with humans comes naturally to everyone.
...
The server is event-driven. Updates come in from three ends: browsers (over WebSocket), the REST API, and the LLM providers (via the agent sessions). Everything funnels into a single event loop, where each event can trigger some combination of: (a) browser updates via WebSocket, (b) agent session updates, and (c) state writes (all the state is stored in `~/.isomux`).[^opencode]

[^opencode]: Isomux now supports OpenCode as a third backend.

<BlogImage
  src="/blog/isomux/architecture.png"
  alt="Isomux system design: two users' devices talk to the Bun service over WebSocket and a REST API; inside the service, an agent lifecycle and event loop run agents through a shared backend abstraction connected to the Claude Agent SDK and Codex App Server; state persists to the local file system (~/.isomux)"
/>

Claude agents use the [Claude Agent SDK](https://docs.anthropic.com/en/docs/claude-code/sdk), Codex agents use the [Codex CLI's App Server](https://github.com/openai/codex), and [OpenCode](https://opencode.ai) agents use a shared OpenCode server. All three sit behind the same backend abstraction. There are multiple ways of making a meta-harness multi-provider; the [appendix](#appendix-three-ways-to-make-a-meta-harness-multi-provider) shows a comparison.
...
<BlogImage
  ...
  alt="A stack of Isomux-tagged tool-call rows in the chat, each a curl call to the API rendered as a labeled card: 'Create task' with title, description, and priority chips; 'Edit Isomuxer3's settings' with modelFamily and effort chips; 'Clear Isomuxer3's conversation'; 'Claim task a65621b0 for Isomuxer3' with an assignee chip; and 'Send a message to Isomuxer3' with a text chip"
/>
...
- Agent memory
- The task board (tasks are scoped to a room, or shared office-wide)
- Usage tracking
...
- a name and look
- a working directory
- the backend (Claude, Codex, or OpenCode), model, permission mode, thinking effort, etc.
...
Isomux has hierarchical memory so agents can share facts about the office and keep facts relevant to just one agent in that agent's memory.
...
Agents can also schedule messages for themselves (or others) in the future. For example, the screenshot below shows an agent using this feature to check on a training run for my [Wall Game AI](/blog/wall-game-ai) every 3h.

<BlogImage
  src="/blog/isomux/scheduled-messages.png"
...
This shows 2,776 messages in a 3-day period.[^wall-game-worker]

- orange is me-to-agent
- purple is agent-to-agent
- yellow circle is turn end (can be thought of as agent-to-me)
- red circle is scheduled self-message

[^wall-game-worker]: An interesting case is "Wall Game Worker 1". It's just monitoring a training run. Checks that it's doing fine, schedules a message for itself a few hours later, repeat.

<BlogImage
  src="/blog/isomux/message-timeline.png"
  alt="A timeline of 2,776 messages between me and 23 agents over three days, Aug 29 to Sep 1 2026. Each agent gets a row: Isomux PM, six Isomux workers paired with six reviewers in a separate room, and the wall game agents below them. Orange curves are messages from me to an agent, purple curves are agent to agent, yellow dots mark the end of a turn, and red dots mark a scheduled self-message. Traffic arrives in dense bursts separated by quiet hours, and the Wall Game Worker 1 row is almost entirely evenly spaced red dots"
/>

Over the 30 days to 2026-09-09:[^messaging-stats]

| Category | Count |
| --- | --- |
| Agent → a different agent (delivered) | 13,024 |
| Agent → itself (scheduled self-reminders) | 1,473 |
| Agent → itself (session handoffs) | 124 |
| **All messages sent through the agent messaging API** | **14,621** |

[^messaging-stats]: Measured 2026-09-09 across all 3,096 conversation logs in `~/.isomux/logs/` (179 agent directories, 687,079 log lines), from 36 different senders to 37 different receivers.

As I refine my workflow, I'll see what changes are needed to make it work and come back to update this section.

### Agent-Human Shared Task Board

Agents and humans share a task board. Anyone can create, assign, claim, and close tasks, from the browser UI or the REST API.

Besides the assignee, tasks can be scoped to a room or the whole office.
...
<Callout>
  Agents often discover improvements or follow-ups while working on a task. They can put those on the shared board for another agent or human to pick up later. This makes handoffs non-linear: one session can leave work for several future sessions, or pick up work from several past sessions.
</Callout>

As of September 17, 2026, my office’s task board holds 310 open tasks and 1,157 closed tasks. I created 381, agents created 1,067, and scheduled runs (cron jobs) created 19. 20+ agents appear as creators, and likewise as assignees. Two humans also appear as assignees. Agents also create tasks for humans: ten recorded tasks go in that direction.[^task-stats]

[^task-stats]: Counts cover retained tasks visible through the task API as of September 17, 2026 (PT); deleted tasks are excluded. Open includes 194 open, 5 in-progress, and 111 backlog tasks; closed means done. The task attributed to “Nil phone (via Agent 3)” is counted as agent-created. Creator and assignee counts conservatively match recorded names to distinct current agents. Assignees are the recorded owners, not necessarily the people or agents who completed the work.
...
There are two user roles: owner and regular member. The split controls who can issue invites, and who can see which rooms.[^access]
...
I self-host my office. For reachability beyond my own VPN, the best path I've found is [Tailscale Funnel](https://isomux.com/docs/access-and-invites).

<Callout>
  I also built a managed version for people who don't want to set up and maintain their own server: [isomux.com/hosted](https://isomux.com/hosted). I cover its provisioning, billing, and architecture in [Building a Hosted Agents Product End-to-End](/blog/hosted-isomux).
</Callout>
...
Here is an office with four people connected, each ghost parked next to the agent they are looking at:
...
Claude and Codex agents share all the same office features. They can message each other. But Codex works quite differently:
...
**Isomux: drive harness APIs.** Isomux runs each provider's harness programmatically and renders one shared UI. It's a middle ground that leans on the provider's own harness for the hard parts while still owning the UI. Since each harness integration surface is different, this approach has the most provider-specific code, requiring the most work per additional provider.
