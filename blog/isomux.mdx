...
In fact, I believe that, as a general principle, any meta-harness should have hierarchical, user-defined system prompts:

<BlogImage
  src="/blog/isomux/hierarchical-prompts.png"
  alt="Isomux: 3 layers of user-defined system prompts (office-wide, per-room, per-agent) concatenated and sent to the agent, alongside the general principle that any meta-harness should expose hierarchical, user-defined system prompts"
/>
...
<Callout>
  Unlike the other features above, this one isn't built yet; it's just a design.
</Callout>

I've been designing a memory system for Isomux.

Just like [system prompts](#hierarchical-prompts), I want memories to follow the same hierarchy — roughly, they can be for a specific agent, a whole room, or the whole office.[^mem0]

**Memory creation**: a hook that runs after every turn. It extracts facts to remember, chooses their scope (agent/room/office), and stores them in a vector DB (like [Qdrant](https://qdrant.tech)). During this step, we must also update or remove preexisting obsolete memories, like [mem0 does](https://docs.mem0.ai/core-concepts/memory-operations/update).
...
<BlogImage
  src="/blog/isomux/memory.png"
  alt="Isomux's planned hierarchical memory flow. Memory creation (after every turn): (1) extract facts from the conversation, (2) choose each fact's scope (agent, room, or office), (3) store it in the matching agent, room, or office memory. Memory retrieval (before every assistant turn): pull relevant memories at each level (agent, room, office), then merge, deduplicate, and rank them by relevance, and pass the result to the agent as context for its turn."
/>

[^mem0]: I experimented with an existing memory solution, [Mem0](https://mem0.ai), but it doesn't fit this hierarchical memory shape. I'll probably roll my own, inspired by their [open-source library](https://github.com/mem0ai/mem0).
