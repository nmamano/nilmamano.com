...
When a browser first connects, it receives a full snapshot of the office containing every the settings of every agent, their logs, and office-wide settings.
...
More details on [isomux.com](https://isomux.com).
...
Given how much I use isomux, common interactions should be easy. The keyboard shortcuts I use the most are:
...
Go to the frontend on your browser, then in the browser menu, find the option "Add to home screen." This turns the website into a "Web App". Here is a [demo](https://x.com/Nil053/status/2039996579965542516/video/1) of what it looks like.
...
Even if nobody else uses Isomux (do so at your own peril, it's not tested beyond my setup) it provides a ton of value to myself. There's no feature pulling me back to raw Claude Code.
...
We'll all be working with agents, so it's important to really like your orchestration tool. The orchestration layer is the new editor.
