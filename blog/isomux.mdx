...
<Callout>
  Every conversation should be [multi-user](#live-user-presence),
  [multi-agent](#agent-to-agent-communication),
  [multi-provider](#how-codex-differs), and [multi-device](#access).
</Callout>
...
<Callout>
  Isomux is being built by agents running inside isomux since 3h after the
  project was started. Developing a dev tool from itself is fun!
</Callout>
...
Each agent gets an individual session history with `/resume` support.
