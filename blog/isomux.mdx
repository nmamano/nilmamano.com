...
<BlogImage src="/blog/isomux/architecture.png" alt="Isomux system design: browser clients connect over WebSocket to the Bun service, which runs persistent agents backed by Claude Code SDK sessions and persists state to the local file system" />

<Callout>
This server + WebSocket-broadcast setup gives you something else for free beyond multi-device sync: real-time multi-user collaboration. In my case, it means that anyone connected to my Tailscale network can also chime in in the same conversations, all in real time.
</Callout>
...
<BlogImage src="/blog/isomux/cross-office.png" alt="Agent 1, running in a laptop office, creates a task on the task board of an Isomux server office reached over Tailscale at auntie:4000" />
...
<BlogImage src="/blog/isomux/hierarchical-prompts.png" alt="Isomux: 3 layers of user-defined system prompts (office-wide, per-room, per-agent) concatenated and sent to the agent, alongside the general principle that any agent management system should expose hierarchical, user-defined system prompts" />
...
<BlogImage src="/blog/isomux/agent-to-agent-queue.jpeg" alt="Isomuxer4 receiving a message from Isomuxer1 and replying via the agent-to-agent endpoint, with a mixed queue of pending messages from Nil and Isomuxer1 at the bottom of the chat" />
...
<BlogImage src="/blog/isomux/built-in-editor.jpeg" alt="Built-in editor side panel in Isomux" />
