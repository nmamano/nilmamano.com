...
The system prompt is designed to be brief, but leaves breadcrumbs so the agent can load in more state if it needs to. See more on the [task board](#shared-task-board) and [image showing](#file-attachments) features below.
