...
coverImage: "/blog/set-and-map-implementations/cover.png"
...
We can then take the fractional part alone, which is between `0` and `1`, and multiply it by `m` to scale it up to the range between `0` and `m`. Finally, we truncate the fractional part and output the integer part, which will be a "random-looking" integer between `0` and `m-1`.[^2]
