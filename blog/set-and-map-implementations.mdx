...
**Prerequisites:** Dynamic Arrays, Sets & Maps. It is recommended to read those chapters from BCtCI first.
...
```
add(k, v):   if k is not in the map, add key k to the map with value v
             if k is already in the map, update its value to v

remove(k):   if k is in the map, remove it from the map

contains(k): return whether the key k is in the map

get(k):      return the value for key k
             if k is not in the map, return a null value
...
add(k, v): adds key k with value v to the multimap, even if key k is  
 already found.  
 remove(k): removes **all** key-value pairs with k as the key.  
 contains(k): returns whether the multimap contains any key-value pair with  
 k as the key  
 get(k): returns **all** values associated to key k in a list. If there is none,  
 returns an empty list.  
 size(): return the number of key-value pairs in the multiset.
...
`h("cbaa") = 99 * 1283 + 98 * 1282 + 97 * 1281 + 97 * 1280 = 209236193`

Then:

1. First, we remove the contribution from the first letter, `'c'`, by subtracting `99 * 1283`. Now, we have:

`98 * 1282 + 97 * 1281 + 97 * 1280 = 1618145`
...
`98 * 1283 + 97 * 1282 + 97 * 1281 = 207122560`

3. Finally, we add the contribution of the new letter, `'b'`, which is `98 * 1280`. We end up with:

`98 * 1283 + 97 * 1282 + 97 * 1281 + 98 * 1280 = 207122658`
...
We pass `128tn-1 % 10^9 + 7` as the `first_power` parameter so that we do not need to compute it for each call to `next_hash()`, as that would increase the runtime. This way, we compute the hashes of all the substrings of length `t` in `O(sn)` time. Putting all the pieces together, we get the rolling hash algorithm:
...
[^4]: See the 'String Manipulation' chapter for more on the ASCII format.
