...
A friend who works in the AI data industry walked me through how frontier models are trained, what RL (Reinforcement Learning) environments are, and what the job actually looks like at some of the top AI startups.

The thesis is that RL environments are the third and latest wave of LLM RL, and the current focus of a lot of investment in AI. He explains how we got here and what's driving the shift.

<Callout title="Disclaimer">
This post is a cleaned up version of his explanation.
It reflects one practitioner's perspective; YMMV.
**Any figures given should be taken as an off-the-cuff estimate, not a researched value.**
</Callout>
...
SFT is the same as process as pre-training, just with different data, so we are not at RL yet.
...
The amount of compute spent on this kind of reasoning RL sometimes approaches pre-training scale, which is known as _scaling post-training_.
...
They created giant datasets of positive and negative examples, and then companies can DPO their models offline.
...
For GRPO to work, we need the model to get it right at least some times, ideally in a middle band (roughly 20-50%). If it's too hard, there's no positive signal to incraese; if it's too easy, training is saturated.
...
Doing a batch of 100 rollouts may take a few minutes.
...
This is the original version that OpenAI used back in the day.
...
Increasingly, the labs care about their models being good at what they call economically valuable tasks. Things like finance, accounting, insurance, and healthcare. Most office work.
...
So far, software engineering has been the main one. These days, models can write code fine. So now the goal is about end-to-end software engineering workflows. SRE, deployment, etc.
...
- First wave - RLHF: "Here's some preference data from human labelers" (Scale AI, DPO).
- Second wave - RLVR: "Here are questions and answers generated by experts." (Expert networks, GRPO).
- Third wave - Agentic RL: "Here are tasks done by real workers." (also GRPO RL).
...
We can't put Microsoft Word in the container, but we can install an open-source variant like LibreOffice. We do this for all software involved in the task.
...
The tasks are designated by human experts, who specify the goal (which becomes the task's starting prompt).
...
<Callout>
For Agentic RL, we need both experts and engineers to set up the environments. That's what some new AI data companies focus on.
</Callout>
