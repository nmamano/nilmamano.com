...
<BlogTable
  caption="**Table 1.** Time and space complexity comparison of different approaches for top-k problems."
  centered={true}
>
  <TableHeader>
    <TableRow>
    ...
    <TableRow>
      ...
      <TableHead className="text-center">Space</TableHead>
      <TableHead className="text-center">Time</TableHead>
      <TableHead className="text-center">Space</TableHead>
    </TableRow>
  </TableHeader>
  <TableBody>
    <TableRow>
      ...
      <TableCell className="text-center">`O(n log n)`</TableCell>
      <TableCell className="text-center">`O(n)`</TableCell>
      <TableCell className="text-center">`O(n log n)`</TableCell>
      <TableCell className="text-center">`O(n)`</TableCell>
    </TableRow>
    <TableRow>
      <TableCell>Min Heap</TableCell>
      <TableCell className="text-center">`O(n + k log n)`</TableCell>
      <TableCell className="text-center">`O(n)`</TableCell>
      <TableCell className="text-center">`O(n + k log n)`</TableCell>
      <TableCell className="text-center">`O(n)`</TableCell>
    </TableRow>
    <TableRow>
      <TableCell>Max Heap of size k</TableCell>
      <TableCell className="text-center">`O(n log k)`</TableCell>
      <TableCell className="text-center">`O(k)`</TableCell>
      <TableCell className="text-center">`O(n log k)`</TableCell>
      <TableCell className="text-center">`O(k)`</TableCell>
    </TableRow>
    <TableRow>
      <TableCell>Quickselect</TableCell>
      <TableCell className="text-center">`O(n)`</TableCell>
      <TableCell className="text-center">`O(n)`</TableCell>
      <TableCell className="text-center">`O(n + k log k)`</TableCell>
      <TableCell className="text-center">`O(n)`</TableCell>
    </TableRow>
    <TableRow>
...
This approach leverages that [we can build a heap in linear time with heapify](./heapify-analysis). After that, we do `k` heap pops. Since the heap has `O(n)` elements, each pop takes `O(log n)` time.
...
_Want to leave a comment? You can post under the [linkedin post](https://www.linkedin.com/feed/update/urn:li:activity:7356869453780832257/) or the [X post](https://x.com/Nil053/status/1950666697020702849)._
