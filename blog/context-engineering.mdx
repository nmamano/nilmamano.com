...
Lately, it seems that "context" is in everyone's minds -- and for good reason. In this post, we'll explain why.
...
And the next word you get **depends on only two things**:
...
refine the weights of a base model by giving it additional training data from
...
For example, RAG (Retrieval-Augmented Generation; more on this below) is a lot more succesful than fine-tuning at reducing hallucinations. RAG changes the context; fine-tuning changes the model. (We can also do both!)
...
[^1]: If you put too much stuff in the context, the quality of the output degrades. This is known as "context rot".
