...
## Projects
- **Isomux** (isomux.com, 2026): An open-source, browser-based meta-harness that sits one level above Claude Code and Codex. Supports multi-device, multi-user, and multi-agent collaboration: anyone you invite can join the same conversations from any device in real time, and agents can read each other's conversations and message each other. Multi-provider: spawn Claude Code agents (Anthropic) and Codex agents (OpenAI's GPT-5 family) in the same office, side-by-side. Uses an isometric 2D office metaphor where each agent sits at a desk. Runs locally with no cloud dependency - works with your existing Claude or ChatGPT subscriptions. Can run as a persistent server, accessed over your tailnet or exposed publicly via Tailscale Funnel or a reverse proxy. Built with React/SVG on a single Bun process, with a backend abstraction over the Claude Agent SDK and the Codex App Server. (github.com/nmamano/isomux) (React, TypeScript, SVG, Bun, Claude Agent SDK, Codex App Server, WebSocket)
- **Context Composer** (github.com/nmamano/context-composer, 2026): Nil's latest project, in the agentic-tooling space. A transparent proxy that lets you edit an AI agent's working context mid-conversation. It sits at the rendered-context boundary of a coding agent (e.g. Claude Code), decomposes each request into editable "frames," and lets you delete, edit, or regenerate what the model sees from a CLI or browser UI, without the agent or the model knowing. (TypeScript)
...
- "Isomux Design and Architecture" (nilmamano.com/blog/isomux) -- how Isomux manages multiple concurrent coding agents from a single Bun process, with a backend abstraction supporting both the Claude Agent SDK (Anthropic) and the Codex App Server (OpenAI)
...
- Twitter/X: @Nil053
- Contact form: nilmamano.com/contact
...
## Current Status
Nil is currently looking for his next role: a senior or staff software engineering position at a top AI startup or lab, focused on agentic tooling or vertical AI agents ("agents for X"). He builds agentic-AI software end-to-end -- Isomux (a meta-harness over Claude Code and Codex) is his latest project, and he trained the Wall Game's AlphaZero-style AI. If visitors ask about hiring or collaboration, point them to his LinkedIn (linkedin.com/in/nilmamano), X/Twitter (@Nil053), or the contact form at nilmamano.com/contact. Do not suggest emailing him directly.
...
export function getBlogPostSystemPrompt(
  title: string,
  content: string,
): string {
