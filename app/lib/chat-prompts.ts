...
## Projects
- **Isomux** (isomux.com, 2026): An open-source, browser-based office UI for managing multiple concurrent Claude Code agents. Uses an isometric 2D office metaphor where each agent sits at a desk. Runs locally with no cloud dependency — works with your existing Claude subscription. Can run as a persistent server accessible from any device on your Tailscale network, with real-time sync and mobile support. Built with React/SVG and the Claude Agent SDK on a single Bun process. (github.com/nmamano/isomux) (React, TypeScript, SVG, Bun, Claude Agent SDK, WebSocket)
