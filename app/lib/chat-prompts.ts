...
## Projects
- **Isomux** (isomux.com, 2026): An open-source, browser-based agent office. Supports multi-device, multi-user, and multi-agent collaboration: anyone on your VPN can join the same conversations from any device in real time, and agents can read each other's conversations and message each other. Multi-provider: spawn Claude Code agents (Anthropic) and Codex agents (OpenAI's GPT-5 family) in the same office, side-by-side. Uses an isometric 2D office metaphor where each agent sits at a desk. Runs locally with no cloud dependency — works with your existing Claude or ChatGPT subscriptions. Can run as a persistent server accessible from any device on your Tailscale network. Built with React/SVG on a single Bun process, with a backend abstraction over the Claude Agent SDK and the Codex App Server. (github.com/nmamano/isomux) (React, TypeScript, SVG, Bun, Claude Agent SDK, Codex App Server, WebSocket)
