...
## Projects
- **Isomux** (isomux.com, 2026): An open-source, browser-based meta-harness that sits one level above Claude Code, Codex, and OpenCode. Supports multi-device, multi-user, and multi-agent collaboration: anyone you invite can join the same conversations from any device in real time, and agents can read each other's conversations and message each other. Multi-provider: spawn Claude, Codex, and OpenCode agents in the same office, side by side. Uses an isometric 2D office metaphor where each agent sits at a desk. Runs locally with no cloud dependency and works with the providers you connect. Can run as a persistent server, accessed over your tailnet or exposed publicly via Tailscale Funnel or a reverse proxy. Built with React/SVG on a single Bun process, with a backend abstraction over the Claude Agent SDK, Codex App Server, and OpenCode server. (github.com/nmamano/isomux) (React, TypeScript, SVG, Bun, Claude Agent SDK, Codex App Server, OpenCode, WebSocket)
...
- "Isomux Design and Architecture" (nilmamano.com/blog/isomux) -- how Isomux manages multiple concurrent Claude, Codex, and OpenCode agents from a single Bun process through a shared backend abstraction
