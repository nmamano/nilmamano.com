...
const concepts = [
  ...
  { id: "study", title: "B · Study" },
  { id: "journal", title: "C · Journal" },
  { id: "helper", title: "D · Window" },
  { id: "spread", title: "Open book" },
  { id: "reader", title: "Chapter pages" },
  { id: "cinematic", title: "Illustrated book" },
];
...
export function Portrait() {
  return <div className={styles.portrait}>
    {/* Keep the original daytime and nighttime photographs. */}
...
export function SocialLinks() {
  return <nav className={styles.socials} aria-label="Profiles and resume">
...
export function LandingConcept({ variant }: { variant: "letter" | "study" | "journal" }) {
  ...
  return <div className={`${styles.preview} ${styles[variant]}`}>
    <nav className={styles.chooser} aria-label="Compare landing page designs">
      ...
      {concepts.map(c => <Link key={c.id} href={`/landing-preview/${c.id}`} aria-current={variant === c.id ? "page" : undefined}>{c.title}</Link>)}
      <button onClick={() => setTheme(resolvedTheme === "dark" ? "light" : "dark")} aria-label="Toggle color theme"><Sun className={styles.sun} size={17} /><Moon className={styles.moon} size={17} /></button>
    </nav>
    <div className={styles.canvas}>
      <header className={styles.siteNav}>
        <span aria-hidden="true" />
        <nav aria-label="Site navigation"><A href="/research">Research</A><A href="/posts">Feed</A><A href="/blog">Blog <span aria-hidden="true">↗</span></A></nav>
      </header>
      <main className={styles.page}>
        ...
        <article className={styles.story} aria-label="About Nil">
          ...
          <div className={styles.chapters}>
            <section className={styles.chapter} aria-label="Data structures and algorithms"><span className={styles.chapterLabel}>01 / Data structures &amp; algorithms</span><Research /></section>
            <section className={styles.chapter} aria-label="Agentic tooling"><span className={styles.chapterLabel}>02 / Working with agents</span><Agents /></section>
          </div>
          <div className={styles.otherWork}><OtherWork /></div>
        </article>
      </main>
      <footer className={styles.footer}><span>© {new Date().getFullYear()} Nil Mamano</span><nav aria-label="More about Nil"><A href="/personal">Outside of work</A><A href="/contact">Get in touch</A><A href="/rss.xml">RSS</A></nav></footer>
    </div>
  </div>;
}
