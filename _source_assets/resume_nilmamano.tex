...
\vspace{-3px}
\begin{description}
	\item[Beyond Cracking the Coding Interview]\quad \href{https://www.amazon.com/dp/195570600X}{amazon.com/dp/195570600X}\hfill 2025
	
	Co-wrote the official sequel to `Cracking the Coding Interview' with Gayle Laakmann McDowell et al. \#1 Best Seller in Data Structures \& Algorithms on Amazon.

	\vspace{2px}
	\noindent Built \href{https://dsatoolkit.com}{dsatoolkit.com}, a companion DS\&A toolkit (Next.js, TypeScript). Built an automated pipeline to parse, test, and generate articles for all 1,440 code solutions across 5 languages.

	\vspace{2px}
	\noindent Co-designed an AI interviewer for the book problems (\href{https://bctci.co/ai}{bctci.co/ai}).

	\vspace{2px}
	\noindent Built an LLM orchestration pipeline to translate 288 book solutions from Python to Go with few-shot prompting and test-driven retry loops, using local models and APIs.
...
\begin{description}
	...
	C++, Go, Python, SQL $|$ Worked on Google's software-defined WAN, which carries traffic between all Google data centers.
...
Co-authored 9 peer-reviewed papers on algorithm design, including as main author in tier A conferences like ICALP and ISAAC. The papers describe new algorithmic improvements in graph theory, computational geometry, and computational biology. %For example: improved the runtime of the multi-fragment algorithm for Euclidean TSP from $O(n^2)$ to $O(n\log n)$.
...
\vspace{2px}
Led a study on the effect of immediate automated feedback on learning outcomes (120 students, experimental/control group).

\item[Undergrad Researcher]\quad \href{https://github.com/nmamano/SANA}{github.com/nmamano/SANA} \hfill Feb 2015 -- Jul 2015

C++ $|$ Created SANA, a biological network alignment tool that uses simulated annealing. 100+ citations; actively maintained for 10+ years by 50+ collaborators, with 2,000+ commits.
\end{description}
...
\begin{description}
	...
	React, TypeScript, Tailwind, shadcn, PostgreSQL, MongoDB, WebSocket $|$ C++, PyTorch, TensorRT, ONNX, WebAssembly $|$ A full-stack online board game supporting AIs as first-class citizens via an AI integration protocol/client. Trained multiple AlphaZero-style models for different board sizes and rule sets via self-play on consumer GPUs.
