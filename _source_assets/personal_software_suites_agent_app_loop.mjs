...
const edges = [
  ...
  { from: "agent", to: "app",   side: +1, gap: 64, lines: ["Implementation"] },
  { from: "app",   to: "agent", side: +1, gap: 60, lines: ["Notifications"] },
  { from: "agent", to: "state", side: +1, rot: true, gap: 46, lines: ["Natural language requests", "mapped to structured", "state updates"] },
...
for (const e of edges) {
  ...
  body += `  <text text-anchor="middle" font-size="13.5" font-weight="500" fill="${color}"${rot}>${spans}</text>\n\n`;
}
